<?php
  ...
  class ModelOutput {
     ...
     * TODO: Corrigir possíveis bugs (copiado de batch para saida)
     */
     ...
     * TODO: Corrigir possíveis bugs (copiado de batch para saida)
     * @param OutputDTO      - classe que possui dados de nota
     * 
     * @return bool|array   - Boa ou má execução
     */
    public function insert(OutputDTO $outputDTO) {
      ...
      try {
        ...
        $stmt->bindParam(':idnota',      $outputDTO->getIdInvoice(),     PDO::PARAM_INT);         // Dando valor ao espaço declarado.
        ...
        if ($stmt->execute()) { 
          ...
          if ($stmt->rowCount() == 1) {
            ...
            $insert_stmt->bindParam(':idnota',      $outputDTO->getIdInvoice(),     PDO::PARAM_INT); // Dando valor ao espaço declarado.
            $insert_stmt->bindParam(':cod',         $outputDTO->getCod(),           PDO::PARAM_INT); // **
            
            // Verifica se há uma boa execução
            if ($insert_stmt->execute()) {

              $text = "[ CREATE ][ OUTPUT ] - [ Nota da Nota-Fiscal ID: {$outputDTO->getIdInvoice()} ]";   // Salva topico para log,
      ...
      //> Tratativa de erro relacionado ao Banco de Dados.
        ...
        $text = ["[ ERROR ][ BD ] - [ OUTPUT / CREATE ]"];                     // Salva topicos para log
      ...
      //> Tratativa de erro relacionado ao Sistema.
        ...
        $text = ["[ ERROR ][ SYSTEN ]","[ MODEL ][ OUTPUT / CREATE ]"];        // Salva topicos para log
     ...
     * @return bool|array   - Boa ou má execução
     */
    public function update(OutputDTO $outputDTO) {
      ...
      try {
        ...
        $stmt  = $this->conn->prepare($query);                                                              // Preparando sintaxe SQL
        $stmt->bindParam('idnota',       $outputDTO->getIdOutput(),       PDO::PARAM_INT);                    // Dando valor ao espaço declarado
        ...
        if ($stmt->execute()) {
          ...
          $update_stmt->bindParam(':idnota',       $outputDTO->getIdOutput(),                PDO::PARAM_INT); // Dando valores aos espaços declarados
          $update_stmt->bindValue(':cod',          $outputDTO->getCod()         ?? null,    PDO::PARAM_STR); // **
          $update_stmt->bindValue(':idnota',       $outputDTO->getIdInvoice()   ?? null,    PDO::PARAM_INT); // **

          if ($update_stmt->execute()) {

            $text = "[ UPDATE ][ OUTPUT ] - [ Atualizando NOTA ID: {$outputDTO->getIdOutput()} ]".             // Salva topicos para log
            "[ ID nota-fiscal referente: {$old_id_invoice} > {$outputDTO->getIdInvoice()} ]";                // Concatenado.
      ...
      //> Tratativa de erro relacionado ao Banco de Dados.
        ...
        $text = ["[ ERROR ][ BD ] - [ OUTPUT / UPDATE ]"];                     // Salva topicos para log
        $log = write_log::write($text, "error-pdo", $error_pdo);              // Chamando função para escrever erro do BD no log
      ...
      //> Tratativa de erro relacionado ao Sistema.
        ...
        $text = ["[ ERROR ][ SYSTEN ]","[ MODEL ][ OUTPUT / UPDATE ]"];        // Salva topicos para log
        $log = write_log::write($text, "error-system", $error_system);        // Chamando função para escrever erro de sistema no log
     ...
     * @return bool|array   - Boa ou má execução
     */
    public function delete(OutputDTO $outputDTO) {
      ...
      try {
        ...
        $stmt->bindParam(':idnota_fk',       $outputDTO->getIdOutput(),         PDO::PARAM_INT);          // Dando valor ao espaço declarado
        ...
        if ($stmt->rowCount() == 0) {
          ...
          $delete_stmt->bindParam(':id_sent',      $outputDTO->getIdOutput(),       PDO::PARAM_INT);      // Dando valor ao espaço declarado
          
          // Verificando se há uma boa execução
          if ($delete_stmt->execute()) {

            $text =["[ DELETE ][ OUTPUT ] - [ ID: {$outputDTO->getIdOutput()} ]"];                         // Salva topicos para log
      ...
      //> Tratativa de erro relacionado ao Banco de Dados.
        ...
        $text = ["[ ERROR ][ BD ] - [ OUTPUT / DELETE ]"];                     // Salva topicos para log
        $log = write_log::write($text, "error-pdo", $error_pdo);              // Chamando função para escrever erro do BD no log
        throw $error_pdo;                                                     // Alerta controller de uma má execução com "1".
      ...
      //> Tratativa de erro relacionado ao Sistema.
        ...
        $text = ["[ ERROR ][ SYSTEN ]","[ MODEL ][ OUTPUT / DELETE ]"];        // Salva topicos para log
        $log = write_log::write($text, "error-system", $error_system);        // Chamando função para escrever erro de sistema no log
        throw $error_system;                                                  // Alerta controller de uma má execução com "1".
      }
    }
    ...
    public function selectAll(): array {
      ...
      try {
        ...
        $query = "SELECT * FROM saida";                            // Montando Query SQL
        $stmt  = $conn->prepare($query);                           // Preparando 
  
        $saidas = [];                                              // Array de saidas para melhor organização das chaves e valores #
        ...
        if ($stmt->execute()) {                                       
          ...
          while ($row = $stmt->fetch(PDO::FETCH_ASSOC)) {                                 
            ...
            $saida = [                                             // Criando um saida inteiro para o array total de saidas #
              "id"            => $row['idregistro'              ], // Encapsulando resultado por sua categoria
              "output_date"   => $row['data_saida'              ], // **
              "idproduto"     => $row['idproduto'               ], // **
              "quantity"      => $row['quantidade'              ], // **
              "idemprestimo"  => $row['emprestimo_idemprestimo' ], // **
              "idsolicitacao" => $row['emprestimo_idsolicitacao'], // **
            ];
  
            $saidas[] = $saida;                                     // Adicionando novo saida no array de saidas #
          }
        ...
        $text = ["[ BD ][ OUTPUT / GET ALL ]"];                                // Salva topicos para log
        ...
        $text = ["[ SYSTEN ]", "[ MODEL ] [ OUTPUT / GET ALL ]"];              // Salva topicos para log
     ...
     * 
     * @return OutputDTO|false Retorna o nota encontrado ou false se não encontrado
     */
    public function selectById(int $id): mixed {
      ...
      try {
        ...
        if ($stmt->execute()) {
          ...
          if ($result) {
            ...
            return new OutputDTO($result['cod'], $result['data_criacao'], $result['idnota']);
          }
        }
      ...
      //> Tratativa de erro relacionado ao Banco de Dados.
        ...
        $text = ["[ BD ][ OUTPUT / GET BY ID ]"];                              // Salva topicos para log
        $log = write_log::write($text, "error-pdo", $error_pdo);              // Solicita função para escrever registro.
      ...
      //> Tratativa de erro relacionado ao Sistema.
        ...
        $text = ["[ SYSTEN ]", "[ MODEL ] [ OUTPUT / GET BY ID ]"];            // Salva topicos para log
        $log = write_log::write($text, 'error-system', $error_system);        // Solicita função para escrever registro.
